...
export function VendasClientPage({ featureCards }: VendasClientPageProps) {
  return (
    <div className="bg-transparent text-white min-h-screen overflow-x-hidden">
      <motion.header 
        className="fixed top-0 left-0 right-0 z-50 bg-black/50 backdrop-blur-md text-primary-foreground p-3 text-center border-b border-primary/20 shadow-[0_0_25px_rgba(255,0,0,0.3)]"
      ...
      <main className="pt-24 pb-16 px-4 md:px-8">
        <motion.section 
            className="text-center flex flex-col items-center space-y-6 min-h-[70vh] justify-center"
            initial="hidden"
            animate="visible"
            variants={containerVariants}
        >
          <motion.div variants={itemVariants}>
            <IconSkull className="w-20 h-20 text-primary" style={{ filter: 'drop-shadow(0 0 15px hsl(var(--primary)))' }} />
          </motion.div>
          <motion.h1 
            variants={itemVariants} 
            className="font-headline text-6xl md:text-8xl lg:text-9xl font-extrabold tracking-tighter leading-tight"
            style={{ textShadow: '0 0 10px rgba(255, 255, 255, 0.1)'}}
          >
            ACESSO OCULTO
          </motion.h1>
          <motion.p variants={itemVariants} className="text-xl md:text-2xl text-muted-foreground max-w-3xl">
            Conteúdos restritos, raros e privados. Uma coleção de materiais que poucos tiveram o privilégio de ver.
          </motion.p>
